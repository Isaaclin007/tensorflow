...
def StockDataPreProcess_AddSuspendBorder(pp_data, c_data_len):
    ...
    dst_index = 0
    c_len = len(src_date_list)
    if (c_data_len < c_len):
        c_len = c_data_len + 1  # 需要多计算一行数据判断 daily upate 新旧数据间是否有停牌
    for iloop in range(c_len):
...
def PPRegionCompute(pp_data, src_col_index, dst_col_index, region_days, c_data_len, np_func):
    c_len = len(pp_data) - (region_days - 1)
    if c_data_len < c_len:
        c_len = c_data_len

    spread_data = np.zeros((c_len, region_days))
    for iloop in range(region_days):
        spread_data[:, iloop] = pp_data[iloop:iloop+c_len, src_col_index]
    
    pp_data[:c_len, dst_col_index] = np_func(spread_data, axis=1)

    # c_len = len(pp_data) - (region_days - 1)
    # if c_data_len < c_len:
    #     c_len = c_data_len
    # for iloop in range(c_len):
    #     pp_data[iloop][dst_col_index] = np_func(pp_data[iloop:iloop+region_days, src_col_index])

# def StockDataPreProcess_V2(df, adj_mode):
#     data_len = len(df)
...
#     StockDataPreProcess_AddSuspendBorder(pp_data, data_len)
#     return pp_data[:data_len - (preprocess_ref_days - 1)]



# 根据已填充的数据计算衍生数据，只计算衍生数据为0的部分，成功返回True, 出错返回False
def StockDataPreProcessCompute(pp_data, use_daily_basic, use_money_flow, use_adj_factor, adj_mode):
    ...
    # 计算需要 compute 的数据范围[0 ~ c_data_len]
    c_data_len = 0
    while c_data_len < data_len:
        if pp_data[c_data_len][PPI_pre_close] != 0.0:
            break
        c_data_len += 1

    # verify
    c_data = pp_data[:c_data_len]
    if np.isnan(c_data).sum() > 0:
        print('Error: StockDataPreProcess invalid data nan')
        return []
    if np.isinf(c_data).sum() > 0:
        print('Error: StockDataPreProcess invalid data inf')
        return []
    if ((c_data[:,PPI_trade_date] <= 0).sum() > 0) or \
            ((c_data[:,PPI_open] <= 0).sum() > 0) or \
            ((c_data[:,PPI_close] <= 0).sum() > 0) or \
            ((c_data[:,PPI_high] <= 0).sum() > 0) or \
            ((c_data[:,PPI_low] <= 0).sum() > 0):
        print('Error: StockDataPreProcess invalid data 0')
        return []

    # adj forward
    if adj_mode == 'f':  # forward
        c_data[:,PPI_open] *= c_data[:,PPI_adj_factor]
        c_data[:,PPI_close] *= c_data[:,PPI_adj_factor]
        c_data[:,PPI_high] *= c_data[:,PPI_adj_factor]
        c_data[:,PPI_low] *= c_data[:,PPI_adj_factor]
    
    # pre_close
    if c_data_len == data_len:
        pp_data[:data_len-1,PPI_pre_close] = pp_data[1:data_len,PPI_close]
        pp_data[data_len-1][PPI_pre_close] = pp_data[data_len-1][PPI_close]
    else:
        pp_data[:c_data_len,PPI_pre_close] = pp_data[1:c_data_len+1,PPI_close]

    # increase
    c_data[:,PPI_open_increase] = (c_data[:,PPI_open] / c_data[:,PPI_pre_close] - 1.0) * 100.0
    c_data[:,PPI_close_increase] = (c_data[:,PPI_close] / c_data[:,PPI_pre_close] - 1.0) * 100.0
    c_data[:,PPI_high_increase] = (c_data[:,PPI_high] / c_data[:,PPI_pre_close] - 1.0) * 100.0
    c_data[:,PPI_low_increase] = (c_data[:,PPI_low] / c_data[:,PPI_pre_close] - 1.0) * 100.0

    # region 5
    r5_len = data_len - 4
    pp_data[:r5_len, PPI_open_5] = pp_data[4:4+r5_len, PPI_open]
    pp_data[:r5_len, PPI_close_5] = pp_data[:r5_len, PPI_close]
    PPRegionCompute(pp_data, PPI_high, PPI_high_5, 5, c_data_len, np.max)
    PPRegionCompute(pp_data, PPI_low, PPI_low_5, 5, c_data_len, np.min)
    PPRegionCompute(pp_data, PPI_vol, PPI_vol_5, 5, c_data_len, np.sum)
    if use_daily_basic:
        PPRegionCompute(pp_data, PPI_turnover_rate_f, PPI_turnover_rate_f_5, 5, c_data_len, np.sum)
    
    # avg
    PPRegionCompute(pp_data, PPI_close, PPI_close_5_avg, 5, c_data_len, np.mean)
    PPRegionCompute(pp_data, PPI_close, PPI_close_10_avg, 10, c_data_len, np.mean)
    PPRegionCompute(pp_data, PPI_close, PPI_close_30_avg, 30, c_data_len, np.mean)
    PPRegionCompute(pp_data, PPI_close, PPI_close_100_avg, 100, c_data_len, np.mean)
    PPRegionCompute(pp_data, PPI_vol, PPI_vol_5_avg, 5, c_data_len, np.mean)
    PPRegionCompute(pp_data, PPI_vol, PPI_vol_10_avg, 10, c_data_len, np.mean)
    PPRegionCompute(pp_data, PPI_vol, PPI_vol_30_avg, 30, c_data_len, np.mean)
    PPRegionCompute(pp_data, PPI_vol, PPI_vol_100_avg, 100, c_data_len, np.mean)
    
    StockDataPreProcess_AddSuspendBorder(pp_data, c_data_len)
    if (data_len - c_data_len) >= (preprocess_ref_days - 1):
        return pp_data
    else:
        return pp_data[:data_len - (preprocess_ref_days - 1)]
...
def StockDataPreProcess(df, adj_mode):
    ...
    # compute
    return StockDataPreProcessCompute(pp_data, use_daily_basic, use_money_flow, use_adj_factor, adj_mode)
