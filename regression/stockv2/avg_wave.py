...
class AvgWave():
    def __init__(self,
        ...
        self.dataset_size = 0
        self.dataset_len = 0
        self.app_setting_name = '%u_%u_%u_%.4f' % (avg_cycle, mode, continue_up_num, cut_loss_ratio)
    ...
    def AppendWaveData(self, pp_data):
        data_len = len(pp_data)
        if data_len == 0:
            return
        wave_data_name = 'wave_status'
        pp_data[wave_data_name] = WS_NONE
        
        avg_name = 'close_%u_avg' % self.avg_cycle
        continue_up_count = 0
    ...
    def TradeTest(self, pp_data, print_trade_record = False, save_data_unit=False):
        ...
        for day_index in reversed(range(0, data_len)):
            if pp_data.loc[day_index, 'wave_status'] == WS_UP:
                next_status = TS_ON
            else:
                next_status = TS_OFF
    ...
    def GetDataset(self, split_date):
        ...
        val_features = test_data[:, :self.feature.feature_size]
        val_labels = test_data[:, self.feature.feature_size]

        return train_features, train_labels, val_features, val_labels, test_data
...
def main(argv):
    ...
    if FLAGS.mode == 'data':
        ...
        o_dl_model.Train(tf, tl, vf, vl, 20)
